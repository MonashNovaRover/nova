...
let 
  ...
  ssh-terminal = target: name: cmd: ''${pkgs.ptyxis}/bin/ptyxis --tab --title="${name} " -x "bash -ic 'ssh -C -t ${target} \"bash -ic \\\"cd $REMOTE_STORE_DIR/bin; ${cmd}; history -s ${cmd}; exec bash -l\\\"\"; history -s \"ssh -t ${target}\"; exec bash -l'"'';
  ssh-nix-terminal = target: shell: name: cmd: ''${pkgs.ptyxis}/bin/ptyxis --tab --title="${name} " -x "bash -ic 'ssh -C -t ${target} \"bash -ic \\\"${shell} --command \\\\\\\"cd $REMOTE_STORE_DIR/bin; ${cmd}; history -s \\\\\\\\\\\\\\\"${cmd}\\\\\\\\\\\\\\\"; exec bash -l\\\\\\\"; history -s \\\\\\\"${shell}\\\\\\\"; exec bash -l\\\"\"; history -s \"ssh -t ${target}\"; exec bash -l'"'';
