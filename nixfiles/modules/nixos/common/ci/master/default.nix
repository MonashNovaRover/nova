...
in
{
  ...
  config = lib.mkIf cfg.enable {
    ...
    sops.templates."github-authorization" = {
      content = ''
        <github_authorization>
          MonashNovaRover = Bearer ${config.sops.placeholder."hydra/github_token"}
        </github_authorization>
        binary_cache_secret_key_file = ${config.sops.placeholder."hydra/binary_cache_secret"}
      '';
      path = "/run/hydra/github_authorization.conf";
      group = "hydra";
      mode = "440";
    };
    ...
    services.hydra = {
      ...
      extraConfig = ''
        Include /run/hydra/github_authorization.conf
        ...
        ${lib.optionalString (config.sops.secrets."hydra/github_token".path != null) ''
          <githubstatus>
    ...
    systemd.services.hydra-setup = {
      ...
      script = ''
        ...
        hydra-create-user nova \
          ...
          --password ${config.sops.secrets."hydra/password".path} \
          --role admin

        # Configure SSH
        install -Dm600 ${config.sops.secrets."hydra/git_ssh_key".path} ~/.ssh/id_rsa
