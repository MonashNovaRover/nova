...
The username is `nova`, and the password is [secret](../external/src/other/secrets/hydra-password.txt).
...
4. Log in to Hydra at _hydra.example.org_. The username is `nova` and the password is [secret](../external/src/other/secrets/hydra-password.txt). You'll need to log in twice - this is due to a layer
