...
class ExcavationConstructionNode(Node):
    ...
    def __init__(self):
        super().__init__("ExcavationConstructionNode")
    ...
    def joystick_l_callback(self, msg):
        ...
        # Update the inputs
        self.scraper_arm_velocity = abs(joystick_l.ax_stick_x)
    ...
    def joystick_r_callback(self, msg):
        ...
        if joystick_r.btn_thumb_l_state == 1 or joystick_r.btn_thumb_d_state == 1:
            ...
            self.tile_placer_velocity = abs( 255 * joystick_l.ax_stick_x )
            self.scraper_arm_velocity = 0
            self.tile_placer_direction = 5 if joystick_l.ax_stick_x >= 0 else 6
        else:
            self.tile_placer_activated = False
            # Update the inputs
            self.scraper_scoop_velocity = abs( 255 * joystick_r.ax_stick_x)
            self.tile_placer_velocity = 0
            self.scraper_scoop_direction = 3 if joystick_r.ax_stick_x >= 0 else 4

    def get_tile_placer_can_commands() {
        tile_placer_data = [0]
        tile_placer_data.push(self.tile_placer_direction)        
        tile_placer_data.push(self.tile_placer_velocity << 4)
        tile_placer_data.push(self.tile_placer_velocity & 0x0F)
        return tile_placer_data
    }

    def get_scraper_can_commands() {
        scraper_arm_data = [0]
        scraper_arm_data.push(self.scraper_arm_direction)       
        scraper_arm_data.push(self.scraper_arm_velocity << 4)
        scraper_arm_data.push(self.scraper_arm_velocity & 0x0F)

        scraper_scoop_data = [0]
        scraper_scoop_data.push(self.scraper_scoop_direction)        
        scraper_scoop_data.push(self.scraper_scoop_velocity << 4)
        scraper_scoop_data.push(self.scraper_scoop_velocity & 0x0F)
