...
void PlaneGenerator::add_height_map(HeightMap height_map) {
    ...
    plane_x_pixels = height_map.pixel_x;
    plane_y_pixels = height_map.pixel_y;
...
void PlaneGenerator::fit_planes() {
    ...
    for (int plane_i = 0; plane_i < num_planes_x; plane_i++) {
        for (int plane_j = 0; plane_j < num_planes_y; plane_j++) {
            ...
            Vec3 centroid = point_sum / these_pts.size();

            double xx=0.0, yy=0.0, zz=0.0, xy=0.0, xz=0.0, yz=0.0;

            for (uint i = 0; i < these_pts.size(); i++) {
                Vec3 p = these_pts[i];
                Vec3 r = p - centroid;
                xx += r.x*r.x;
                xy += r.x*r.y;
            ...
            Plane plane(these_pts, centroid, normal);
...
float PlaneGenerator::get(int x, int y) {
    return height_map.get(x * 2.5, y * 2.5);
}



Plane::Plane(std::vector<Vec3> points, Vec3 centroid, Vec3 normal) 
    : points(points), centroid(centroid), normal(normal) {}
...
HeightMap steep_cliff_terrain(){
    HeightMap h = HeightMap(MAP_SIZE_X, MAP_SIZE_Y, 2.5);
...
int main() {
    ...
    PlaneGenerator p = PlaneGenerator(800, 800);
    p.fit_planes(h);
