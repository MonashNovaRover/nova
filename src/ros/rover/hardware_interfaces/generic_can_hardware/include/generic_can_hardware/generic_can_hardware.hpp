...
#ifndef GENERIC_CAN_HARDWARE__GENERIC_CAN_HARDWARE_HPP_
#define GENERIC_CAN_HARDWARE__GENERIC_CAN_HARDWARE_HPP_
...
enum class GenericCANSendCommand {
...
enum class GenericCANReceiveCommand{
    ERR_WARN_INF = 0x0,
    CONFIG_DATA = 0x9,
    WRITE_CONFIRMATION = 0xA
};

enum class GenericCANConfigCommand{
    HAS_RESOLVER    = 0x0,
    KP_CURRENT      = 0x1,
    KI_CURRENT      = 0x2,
    MAX_CURRENT     = 0x3,
...
class GenericCANHardware : public hardware_interface::SystemInterface
{
public:
...
private:
    std::string GenericCANHardwareLoggerName;
    ...
    /// @brief      Get a configuration value from the GenericCAN
    /// @param      command - The config value to get
    /// @returns    The optional with config value if received, empty optional otherwise
    template<typename T>
    std::optional<T> get_config(GenericCANConfigCommand command);

    /// @brief      Create the can ID for a given GenericCANSendCommand
    /// @param      command - The command to send
    /// @returns    The can ID
    uint32_t make_can_id(GenericCANSendCommand command) const;

    /// @brief      Create the can ID for a given GenericCANReceiveCommand
    /// @param      command - The command to send
    /// @returns    The can ID
    uint32_t make_can_id(GenericCANReceiveCommand command) const;
...
#endif  // GENERIC_CAN_HARDWARE__GENERIC_CAN_HARDWARE_HPP_
