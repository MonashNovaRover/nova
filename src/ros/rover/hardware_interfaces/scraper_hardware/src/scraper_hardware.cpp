...
#include "scraper_hardware/scraper_hardware.hpp"
...
{
hardware_interface::CallbackReturn ScraperHardware::on_init(
        const hardware_interface::HardwareInfo & info)
{
    ...
    if (canbus_search == info_.hardware_parameters.end()){
        RCLCPP_FATAL(rclcpp::get_logger(ScraperHardwareLoggerName), "No canbus provided");
        return CallbackReturn::ERROR;
    }

    can_device_ = canbus_search->second;
    RCLCPP_INFO_STREAM(rclcpp::get_logger(ScraperHardwareLoggerName),
    ...
    if (canid_search == info_.hardware_parameters.end()){
        RCLCPP_FATAL(rclcpp::get_logger(ScraperHardwareLoggerName), "No canid provided");
        return CallbackReturn::ERROR;
    }

    can_id_ = std::stoul(canid_search->second);
    
    RCLCPP_INFO(rclcpp::get_logger(ScraperHardwareLoggerName), "Using can id %d", can_id_);

    auto clock_rate_search = info_.hardware_parameters.find("clock_rate");
    if (clock_rate_search == info_.hardware_parameters.end()){
        RCLCPP_FATAL(rclcpp::get_logger(ScraperHardwareLoggerName), "No clock rate provided");
        return CallbackReturn::ERROR;
    }
    ...
    if (revolution_pulses_search == info_.hardware_parameters.end()){
        RCLCPP_FATAL(rclcpp::get_logger(ScraperHardwareLoggerName), "No revolution pulses provided");
        return CallbackReturn::ERROR;
    }
    ...
    if (gear_ratio_search == info_.hardware_parameters.end()){
        RCLCPP_FATAL(rclcpp::get_logger(ScraperHardwareLoggerName), "No gear ratio provided");
        return CallbackReturn::ERROR;
    }
    ...
    control_mode_ = scraper_hardware::ControlMode::Undefined;
...
hardware_interface::CallbackReturn ScraperHardware::on_configure(
        const rclcpp_lifecycle::State & previous_state)
{
  // open the can bus
    try {
        bus_->open(can_device_.c_str());
        RCLCPP_INFO(rclcpp::get_logger(ScraperHardwareLoggerName), "Opened canbus on device %s",
                    can_device_.c_str());
    } catch (std::exception &e) {
        RCLCPP_FATAL(rclcpp::get_logger(ScraperHardwareLoggerName), "Failed to start canbus with error: %s",
                     e.what());
        return CallbackReturn::ERROR;
    }
        ...
        //get min_interval
            if (hw_velocity_.state.has_value()) {
                RCLCPP_INFO_STREAM(rclcpp::get_logger(ScraperHardwareLoggerName),
                                   "Getting min interval on Scraper " << can_id_);
                auto min_interval = get_config<uint16_t>(ScraperConfigCommand::MIN_INTERVAL);

                if (min_interval.has_value()) {
                    min_interval_ = min_interval.value();
                    RCLCPP_INFO_STREAM(rclcpp::get_logger(ScraperHardwareLoggerName),
                                       "Min interval on Scraper " << can_id_ << " is " << min_interval_);
                } else {
                    RCLCPP_FATAL_STREAM(rclcpp::get_logger(ScraperHardwareLoggerName),
                                        "Error getting min interval on Scraper " << can_id_);
                    return CallbackReturn::ERROR;
                }
        }
        ...
        if (hw_position_.state.has_value() || hw_position_.command.has_value()) {
            RCLCPP_INFO_STREAM(rclcpp::get_logger(ScraperHardwareLoggerName),
                               "Checking for resolver on Scraper " << can_id_);

            auto resolver_check = get_config<uint16_t>(ScraperConfigCommand::HAS_RESOLVER);
            if (resolver_check.has_value()) {
                if (resolver_check.value()) {
                    RCLCPP_INFO_STREAM(rclcpp::get_logger(ScraperHardwareLoggerName),
                                       "Resolver detected on Scraper " << can_id_);
                    return CallbackReturn::SUCCESS;
                } else {
                    RCLCPP_FATAL_STREAM(rclcpp::get_logger(ScraperHardwareLoggerName),
                                        "No resolver detected on Scraper " << can_id_);
                    return CallbackReturn::ERROR;
                }
            }
            RCLCPP_FATAL_STREAM(rclcpp::get_logger(ScraperHardwareLoggerName),
                                "Error with resolver request on Scraper" << can_id_);
            return CallbackReturn::ERROR;
...
template<typename T>
std::optional<T> ScraperHardware::get_config(ScraperConfigCommand command) {

    const leigh::jcan::Frame min_interval_request = {
            make_can_id(ScraperSendCommand::GET_CONFIG),
    ...
    while (std::chrono::steady_clock::now() - start < std::chrono::seconds(1)) {
        try {
            ...
            if (frame.id == make_can_id(ScraperReceiveCommand::CONFIG_DATA)) {
...
std::vector<hardware_interface::StateInterface> ScraperHardware::export_state_interfaces()
{
...
std::vector<hardware_interface::CommandInterface> ScraperHardware::export_command_interfaces()
{
...
hardware_interface::CallbackReturn ScraperHardware::on_activate(
        const rclcpp_lifecycle::State & /*previous_state*/)
{
...
hardware_interface::CallbackReturn ScraperHardware::on_deactivate(
    const rclcpp_lifecycle::State & /*previous_state*/)
{
...
hardware_interface::return_type ScraperHardware::read(
        const rclcpp::Time & time, const rclcpp::Duration & period)
{
    bus_->spin();
    if(integrate_velocity_ && hw_position_.state.has_value() && hw_velocity_.state.has_value()){
        //RCLCPP_INFO_STREAM(rclcpp::get_logger(ScraperHardwareLoggerName), "Velocity state: " << hw_velocity_.state.value()
        ...
        //RCLCPP_INFO_STREAM(rclcpp::get_logger(ScraperHardwareLoggerName), "New position state: " << hw_position_.state.value());

    }
    return hardware_interface::return_type::OK;
}

hardware_interface::return_type ScraperHardware::write(
        const rclcpp::Time & /*time*/, const rclcpp::Duration & /*period*/)
{
    switch(control_mode_){
        case scraper_hardware::ControlMode::Undefined:
            break;
        case scraper_hardware::ControlMode::Position:
            if (hw_position_.command.has_value()) {
                RCLCPP_DEBUG_STREAM(rclcpp::get_logger(ScraperHardwareLoggerName),
                                   "Sending Position Command " << hw_position_.command.value());
                send_scaled<int16_t>(make_can_id(ScraperSendCommand::DRIVE_POSITION),
                                     hw_position_.command.value() * reversed_multiplier_, hw_position_.max);
            } else {
                RCLCPP_FATAL(rclcpp::get_logger(ScraperHardwareLoggerName), "No position command");
                return hardware_interface::return_type::ERROR;
            }
            break;
        case scraper_hardware::ControlMode::Velocity:
            if (hw_velocity_.command.has_value()) {
               RCLCPP_DEBUG_STREAM(rclcpp::get_logger(ScraperHardwareLoggerName),
                                  "Sending velocity command " << hw_velocity_.command.value() * reversed_multiplier_);
                send_scaled<int16_t>(make_can_id(ScraperSendCommand::DRIVE_VELOCITY),
                                     hw_velocity_.command.value() * reversed_multiplier_, hw_velocity_.max);
            } else {
                RCLCPP_FATAL(rclcpp::get_logger(ScraperHardwareLoggerName), "No velocity command");
                return hardware_interface::return_type::ERROR;
            }
            break;
        case scraper_hardware::ControlMode::Effort:
            if (hw_effort_.command.has_value()) {
                send_scaled<int16_t>(make_can_id(ScraperSendCommand::DRIVE_CURRENT),
                                     hw_effort_.command.value() * reversed_multiplier_, hw_effort_.max);
            } else {
                RCLCPP_FATAL(rclcpp::get_logger(ScraperHardwareLoggerName), "No effort command");
                return hardware_interface::return_type::ERROR;
            }
            break;
    }
...
hardware_interface::return_type
    ScraperHardware::prepare_command_mode_switch(const std::vector<std::string> &start_interfaces,
...
bool ScraperHardware::stop_interface(const std::string &interface){
    ...
    if (interface_name == hardware_interface::HW_IF_POSITION) {
        if (control_mode_ == scraper_hardware::ControlMode::Position) {
            control_mode_ = scraper_hardware::ControlMode::Undefined;
            return true;
        } else {
            RCLCPP_FATAL_STREAM(rclcpp::get_logger(ScraperHardwareLoggerName),
    ...
    if (interface_name == hardware_interface::HW_IF_VELOCITY) {
        if (control_mode_ == scraper_hardware::ControlMode::Velocity) {
    ...
    if (interface_name == hardware_interface::HW_IF_EFFORT) {
        if (control_mode_ == scraper_hardware::ControlMode::Effort) {
...
bool ScraperHardware::start_interface(const std::string &interface){
    ...
    if(control_mode_ != scraper_hardware::ControlMode::Undefined){
        RCLCPP_FATAL_STREAM(rclcpp::get_logger(ScraperHardwareLoggerName),
    ...
    if (interface_name == hardware_interface::HW_IF_POSITION){
        control_mode_ = scraper_hardware::ControlMode::Position;
    } else if (interface_name == hardware_interface::HW_IF_VELOCITY){
        control_mode_ = scraper_hardware::ControlMode::Velocity;
    } else if (interface_name == hardware_interface::HW_IF_EFFORT){
        control_mode_ = scraper_hardware::ControlMode::Effort;
    } else {
        RCLCPP_FATAL_STREAM(rclcpp::get_logger(ScraperHardwareLoggerName),
                            "Unexpected interface " << interface_name.c_str());
        return false;
...
bool ScraperHardware::set_control_interface(
    ...
    if(interface_info.name == hardware_interface::HW_IF_POSITION){
        ...
        if (command){
            ...
            if (resolver_reduction_search == info_.joints[0].parameters.end()){
                RCLCPP_FATAL(rclcpp::get_logger(ScraperHardwareLoggerName), "No resolver reduction provided");
                return false;
            }
        ...
        RCLCPP_FATAL(rclcpp::get_logger(ScraperHardwareLoggerName), "Unexpected interface %s",
    ...
    void ScraperHardware::can_setup() {
        std::vector<uint32_t> ids = {make_can_id(ScraperReceiveCommand::CONFIG_DATA)};
	...
	bus_->set_id_filter(ids);
        if (hw_velocity_.state.has_value() || hw_effort_.state.has_value()) {
            ...
            bus_->add_callback_to(make_can_id(TelemetryPacket::PACKET_1), this, &ScraperHardware::packet_1_callback);
        }
        if (hw_position_.state.has_value() && !integrate_velocity_) {
            RCLCPP_INFO_STREAM(rclcpp::get_logger(ScraperHardwareLoggerName),
                               "Adding packet 3 callback to ID:" << make_can_id(TelemetryPacket::PACKET_3));
            bus_->add_callback_to(make_can_id(TelemetryPacket::PACKET_3), this, &ScraperHardware::packet_3_callback);
        }
        bus_->set_callbacks_enabled(false);
   }

    uint32_t ScraperHardware::make_can_id(ScraperSendCommand command) const
    {
        return static_cast<uint32_t>(CanIdPrefix::SEND) << 8 | can_id_ << 4 |
               static_cast<uint32_t>(command);
    }

    uint32_t ScraperHardware::make_can_id(ScraperReceiveCommand command) const
    {
        return static_cast<uint32_t>(CanIdPrefix::RECEIVE) << 8 | can_id_ << 4 |
               static_cast<uint32_t>(command);
    }

    uint32_t ScraperHardware::make_can_id(TelemetryPacket packet) const
    {
        return static_cast<uint32_t>(CanIdPrefix::RECEIVE) << 8 | can_id_ << 4|
               static_cast<uint32_t>(packet);
    }

    void ScraperHardware::packet_1_callback(leigh::jcan::Frame frame) {
        if(hw_velocity_.state.has_value()) {
    ...
    void ScraperHardware::packet_3_callback(leigh::jcan::Frame frame) {
        if(hw_position_.state.has_value()) {
    ...
    template<typename T>
    double ScraperHardware::convert_scaled(const uint8_t *bytes, double max) {
        return static_cast<double>(from_bytes<T>(bytes))/ std::numeric_limits<T>::max() * max;
    }

    template<typename T>
    T ScraperHardware::from_bytes(const uint8_t *bytes) {
        T data = bytes[0];
    ...
    template<typename T>
    void ScraperHardware::send_scaled(uint32_t id, double value, double max) {
        T data = static_cast<T>( (abs(value) > max ? (value > 0 ? 1 : -1) : value/max)* std::numeric_limits<T>::max());
        send_raw(id, data);
    }

    template<typename T>
    void ScraperHardware::send_raw(const uint32_t id, T data) {
...
PLUGINLIB_EXPORT_CLASS(
  scraper_hardware::ScraperHardware, hardware_interface::SystemInterface)
