...
#ifndef AUGER_HARDWARE__AUGER_HARDWARE_HPP_
#define AUGER_HARDWARE__AUGER_HARDWARE_HPP_
...
enum class AugerReceiveCommand{
    ERR_WARN_INF = 0x0,
    CONFIG_DATA = 0x9,
    WRITE_CONFIRMATION = 0xA
};

enum class AugerConfigCommand{
    HAS_RESOLVER    = 0x0,
    KP_CURRENT      = 0x1,
    KI_CURRENT      = 0x2,
    MAX_CURRENT     = 0x3,
...
private:
    std::string AugerHardwareLoggerName;
    ...
    /// @brief      Get a configuration value from the Auger
    /// @param      command - The config value to get
    /// @returns    The optional with config value if received, empty optional otherwise
    template<typename T>
    std::optional<T> get_config(AugerConfigCommand command);

    /// @brief      Create the can ID for a given AugerSendCommand
    /// @param      command - The command to send
    /// @returns    The can ID
    uint32_t make_can_id(AugerSendCommand command) const;

    /// @brief      Create the can ID for a given AugerReceiveCommand
    /// @param      command - The command to send
    /// @returns    The can ID
    uint32_t make_can_id(AugerReceiveCommand command) const;
...
#endif  // AUGER_HARDWARE__AUGER_HARDWARE_HPP_
