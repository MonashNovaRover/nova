        ...
        const hardware_interface::HardwareInfo & info)
{
    ...
    if (clock_rate_search == info_.hardware_parameters.end()){
        RCLCPP_FATAL(rclcpp::get_logger(QCMDHardwareLoggerName), "No clock rate provided");
        return CallbackReturn::ERROR;
    }
    ...
    if (revolution_pulses_search == info_.hardware_parameters.end()){
        RCLCPP_FATAL(rclcpp::get_logger(QCMDHardwareLoggerName), "No revolution pulses provided");
        return CallbackReturn::ERROR;
    }
    ...
    if (gear_ratio_search == info_.hardware_parameters.end()){
        RCLCPP_FATAL(rclcpp::get_logger(QCMDHardwareLoggerName), "No gear ratio provided");
        return CallbackReturn::ERROR;
    }
    ...
    control_mode_ = qcmd_hardware::ControlMode::Undefined;
    ...
    try {
        ...
        //get min_interval
            if (hw_velocity_.state.has_value()) {
                RCLCPP_INFO_STREAM(rclcpp::get_logger(QCMDHardwareLoggerName),
                                   "Getting min interval on QCMD " << can_id_);
                auto min_interval = get_config<uint16_t>(QCMDConfigCommand::MIN_INTERVAL);

                if (min_interval.has_value()) {
                    min_interval_ = min_interval.value();
                    RCLCPP_INFO_STREAM(rclcpp::get_logger(QCMDHardwareLoggerName),
                                       "Min interval on QCMD " << can_id_ << " is " << min_interval_);
                } else {
                    RCLCPP_FATAL_STREAM(rclcpp::get_logger(QCMDHardwareLoggerName),
                                        "Error getting min interval on QCMD " << can_id_);
                    return CallbackReturn::ERROR;
                }
        }
        ...
        if (hw_position_.state.has_value() || hw_position_.command.has_value()) {
            RCLCPP_INFO_STREAM(rclcpp::get_logger(QCMDHardwareLoggerName),
                               "Checking for resolver on QCMD " << can_id_);

            auto resolver_check = get_config<uint16_t>(QCMDConfigCommand::HAS_RESOLVER);
            if (resolver_check.has_value()) {
                if (resolver_check.value()) {
                    RCLCPP_INFO_STREAM(rclcpp::get_logger(QCMDHardwareLoggerName),
                                       "Resolver detected on QCMD " << can_id_);
                    return CallbackReturn::SUCCESS;
                } else {
                    RCLCPP_FATAL_STREAM(rclcpp::get_logger(QCMDHardwareLoggerName),
                                        "No resolver detected on QCMD " << can_id_);
                    return CallbackReturn::ERROR;
                }
            }
            RCLCPP_FATAL_STREAM(rclcpp::get_logger(QCMDHardwareLoggerName),
                                "Error with resolver request on QCMD" << can_id_);
            return CallbackReturn::ERROR;
...
template<typename T>
std::optional<T> QCMDHardware::get_config(QCMDConfigCommand command) {

    const leigh::jcan::Frame min_interval_request = {
            make_can_id(QCMDSendCommand::GET_CONFIG),
    ...
    while (std::chrono::steady_clock::now() - start < std::chrono::seconds(1)) {
        try {
            ...
            if (frame.id == make_can_id(QCMDReceiveCommand::CONFIG_DATA)) {
        ...
        const rclcpp::Time & time, const rclcpp::Duration & period)
{
    ...
    if(integrate_velocity_ && hw_position_.state.has_value() && hw_velocity_.state.has_value()){
        //RCLCPP_INFO_STREAM(rclcpp::get_logger(QCMDHardwareLoggerName), "Velocity state: " << hw_velocity_.state.value()
        ...
        //RCLCPP_INFO_STREAM(rclcpp::get_logger(QCMDHardwareLoggerName), "New position state: " << hw_position_.state.value());
        ...
        const rclcpp::Time & /*time*/, const rclcpp::Duration & /*period*/)
{
    switch(control_mode_){
        case qcmd_hardware::ControlMode::Undefined:
            break;
        case qcmd_hardware::ControlMode::Position:
            if (hw_position_.command.has_value()) {
                RCLCPP_DEBUG_STREAM(rclcpp::get_logger(QCMDHardwareLoggerName),
                                   "Sending Position Command " << hw_position_.command.value());
                send_scaled<int16_t>(make_can_id(QCMDSendCommand::DRIVE_POSITION),
                                     hw_position_.command.value() * reversed_multiplier_, hw_position_.max);
            } else {
                RCLCPP_FATAL(rclcpp::get_logger(QCMDHardwareLoggerName), "No position command");
                return hardware_interface::return_type::ERROR;
            }
            break;
        case qcmd_hardware::ControlMode::Velocity:
            if (hw_velocity_.command.has_value()) {
               RCLCPP_DEBUG_STREAM(rclcpp::get_logger(QCMDHardwareLoggerName),
                                  "Sending velocity command " << hw_velocity_.command.value() * reversed_multiplier_);
                send_scaled<int16_t>(make_can_id(QCMDSendCommand::DRIVE_VELOCITY),
                                     hw_velocity_.command.value() * reversed_multiplier_, hw_velocity_.max);
            } else {
                RCLCPP_FATAL(rclcpp::get_logger(QCMDHardwareLoggerName), "No velocity command");
                return hardware_interface::return_type::ERROR;
            }
            break;
        case qcmd_hardware::ControlMode::Effort:
            if (hw_effort_.command.has_value()) {
                send_scaled<int16_t>(make_can_id(QCMDSendCommand::DRIVE_CURRENT),
...
bool QCMDHardware::stop_interface(const std::string &interface){
    ...
    if (interface_name == hardware_interface::HW_IF_POSITION) {
        if (control_mode_ == qcmd_hardware::ControlMode::Position) {
            control_mode_ = qcmd_hardware::ControlMode::Undefined;
            return true;
        } else {
            RCLCPP_FATAL_STREAM(rclcpp::get_logger(QCMDHardwareLoggerName),
    ...
    if (interface_name == hardware_interface::HW_IF_VELOCITY) {
        if (control_mode_ == qcmd_hardware::ControlMode::Velocity) {
    ...
    if (interface_name == hardware_interface::HW_IF_EFFORT) {
        if (control_mode_ == qcmd_hardware::ControlMode::Effort) {
...
bool QCMDHardware::start_interface(const std::string &interface){
    ...
    if(control_mode_ != qcmd_hardware::ControlMode::Undefined){
        RCLCPP_FATAL_STREAM(rclcpp::get_logger(QCMDHardwareLoggerName),
    ...
    if (interface_name == hardware_interface::HW_IF_POSITION){
        control_mode_ = qcmd_hardware::ControlMode::Position;
    } else if (interface_name == hardware_interface::HW_IF_VELOCITY){
        control_mode_ = qcmd_hardware::ControlMode::Velocity;
    } else if (interface_name == hardware_interface::HW_IF_EFFORT){
        control_mode_ = qcmd_hardware::ControlMode::Effort;
    } else {
        RCLCPP_FATAL_STREAM(rclcpp::get_logger(QCMDHardwareLoggerName),
                            "Unexpected interface " << interface_name.c_str());
        return false;
...
bool QCMDHardware::set_control_interface(
    ...
    if(interface_info.name == hardware_interface::HW_IF_POSITION){
        ...
        if (command){
            ...
            if (resolver_reduction_search == info_.joints[0].parameters.end()){
                RCLCPP_FATAL(rclcpp::get_logger(QCMDHardwareLoggerName), "No resolver reduction provided");
                return false;
            }
    ...
    void QCMDHardware::can_setup() {
        std::vector<uint32_t> ids = {make_can_id(QCMDReceiveCommand::CONFIG_DATA)};
	...
	bus_->set_id_filter(ids);
        if (hw_velocity_.state.has_value() || hw_effort_.state.has_value()) {
            ...
            bus_->add_callback_to(make_can_id(TelemetryPacket::PACKET_1), this, &QCMDHardware::packet_1_callback);
        }
        if (hw_position_.state.has_value() && !integrate_velocity_) {
            RCLCPP_INFO_STREAM(rclcpp::get_logger(QCMDHardwareLoggerName),
                               "Adding packet 3 callback to ID:" << make_can_id(TelemetryPacket::PACKET_3));
            bus_->add_callback_to(make_can_id(TelemetryPacket::PACKET_3), this, &QCMDHardware::packet_3_callback);
        }
        bus_->set_callbacks_enabled(false);
   }

    uint32_t QCMDHardware::make_can_id(QCMDSendCommand command) const
    {
        return static_cast<uint32_t>(CanIdPrefix::SEND) << 8 | can_id_ << 4 |
               static_cast<uint32_t>(command);
    }

    uint32_t QCMDHardware::make_can_id(QCMDReceiveCommand command) const
    {
        return static_cast<uint32_t>(CanIdPrefix::RECEIVE) << 8 | can_id_ << 4 |
               static_cast<uint32_t>(command);
    }

    uint32_t QCMDHardware::make_can_id(TelemetryPacket packet) const
    {
        return static_cast<uint32_t>(CanIdPrefix::RECEIVE) << 8 | can_id_ << 4|
               static_cast<uint32_t>(packet);
    }

    void QCMDHardware::packet_1_callback(leigh::jcan::Frame frame) {
        if(hw_velocity_.state.has_value()) {
    ...
    void QCMDHardware::packet_3_callback(leigh::jcan::Frame frame) {
        if(hw_position_.state.has_value()) {
