...
#include "nova_path_planner_old/nova_path_planner.hpp"
...
namespace nova_path_planner_old
{
  ...
  NovaPathPlannerOld::NovaPathPlannerOld() : controller_interface::ControllerInterface() {}

  controller_interface::CallbackReturn NovaPathPlannerOld::on_init()
  {
    try
    {
...
//        "moveit_core", "planning_interface::PlannerManager");
  ...
  InterfaceConfiguration NovaPathPlannerOld::command_interface_configuration() const
  {
  ...
  InterfaceConfiguration NovaPathPlannerOld::state_interface_configuration() const
  {
    std::vector<std::string> conf_names;
  ...
  controller_interface::return_type NovaPathPlannerOld::update(const rclcpp::Time &time, const rclcpp::Duration &period)
  {
  ...
  controller_interface::CallbackReturn NovaPathPlannerOld::on_configure(const rclcpp_lifecycle::State&)
  {
    auto logger = get_node()->get_logger();
  ...
  std::string NovaPathPlannerOld::construct_srdf_fallback_string(const urdf::ModelInterfaceSharedPtr &urdf_model,
  ...
  controller_interface::CallbackReturn NovaPathPlannerOld::on_activate(const rclcpp_lifecycle::State&)
  {
    auto logger = get_node()->get_logger();
    ...
    */
    ...
    action_server_ = rclcpp_action::create_server<ArmPlanPath>(
      get_node(),
      params_.action_name,
      std::bind(&NovaPathPlannerOld::handle_action_goal, this, _1, _2),
      std::bind(&NovaPathPlannerOld::handle_action_cancelled, this, _1),
      std::bind(&NovaPathPlannerOld::handle_action_accepted, this, _1));
  ...
  controller_interface::CallbackReturn NovaPathPlannerOld::on_deactivate(const rclcpp_lifecycle::State&)
  {
  ...
  controller_interface::CallbackReturn NovaPathPlannerOld::on_cleanup(const rclcpp_lifecycle::State&)
  {
  ...
  controller_interface::CallbackReturn NovaPathPlannerOld::on_error(const rclcpp_lifecycle::State&)
  {
  ...
  controller_interface::CallbackReturn NovaPathPlannerOld::on_shutdown(const rclcpp_lifecycle::State&)
  {
    return controller_interface::CallbackReturn::SUCCESS;
  }

  void NovaPathPlannerOld::halt()
  {
    // TODO: Implement. Could be useful for some safety features?
  }

  // void NovaPathPlannerOld::publish_to_tf2(const rclcpp::Time &time, const Eigen::Isometry3d& pose) {
  ...
  controller_interface::CallbackReturn NovaPathPlannerOld::configure_joints() {
    auto logger = get_node()->get_logger();
  ...
  std::vector<double> NovaPathPlannerOld::get_state_pos_values() {
  ...
  std::vector<double> NovaPathPlannerOld::get_state_pos_values_non_rt() {
  ...
  rclcpp::Node::SharedPtr NovaPathPlannerOld::create_compat_node_from_lifecycle(
  ...
  std::string NovaPathPlannerOld::joint_to_command_interface_name(const std::string& joint_name) const {
  ...
  bool NovaPathPlannerOld::check_path_for_self_intersection(const std::vector<double> &seed_state,
  ...
  bool NovaPathPlannerOld::check_pose_for_self_intersection(const std::vector<double> &joint_positions) {
  ...
  void NovaPathPlannerOld::generate_allowed_collision_matrix() {
    auto logger = get_node()->get_logger();
  ...
  rclcpp_action::GoalResponse NovaPathPlannerOld::handle_action_goal(const rclcpp_action::GoalUUID &uuid,
  ...
  void NovaPathPlannerOld::handle_action_accepted(const std::shared_ptr<GoalHandleArmPlanPath>& goal_handle) {
    // this needs to return quickly to avoid blocking the executor, so spin up a new thread
    auto logger = get_node()->get_logger();
    RCLCPP_INFO(logger, "Spinning up new thread to handle the action!");
    std::thread{std::bind(&nova_path_planner_old::NovaPathPlannerOld::execute_action, this, _1), goal_handle}.detach();
  }

  rclcpp_action::CancelResponse NovaPathPlannerOld::handle_action_cancelled(const std::shared_ptr<GoalHandleArmPlanPath>& goal_handle) {
    auto logger = get_node()->get_logger();
  ...
  void NovaPathPlannerOld::execute_action(const std::shared_ptr<GoalHandleArmPlanPath> goal_handle) {
    auto logger = get_node()->get_logger();
  ...
  bool NovaPathPlannerOld::try_get_pose_from_forward_kinematics(const std::vector<double> joint_positions,
    ...
    Eigen::fromMsg(fk_poses[0], result);
    return true;
  }

  bool NovaPathPlannerOld::generate_path(Eigen::Isometry3d &start, Eigen::Isometry3d &end,
  ...
  void NovaPathPlannerOld::clear_path_ptr() {
    {
      std::unique_lock<std::mutex> lock(path_mutex_);
  ...
  inline Eigen::Vector3d NovaPathPlannerOld::lerp(const Vector3d& a, const Vector3d& b, const double &t) {
    return (1-t)*a + t*b;
  }

  inline Eigen::Vector3d NovaPathPlannerOld::lerp2(const Vector3d& a, const Vector3d& b, const Vector3d& c, const double &t) {
    return lerp(lerp(a,b, t), lerp(b,c, t), t);
  }

  inline Eigen::Vector3d NovaPathPlannerOld::lerp3(const Vector3d& a, const Vector3d& b, const Vector3d& c, const Vector3d& d, const double &t) {
    return lerp(lerp2(a,b,c, t), lerp2(b,c,d, t), t);
  }

  inline Eigen::Quaterniond NovaPathPlannerOld::slerp(const Eigen::Quaterniond& a, const Eigen::Quaterniond& b, const double &t) {
    return a.slerp(t, b);
  }

  inline Eigen::Quaterniond NovaPathPlannerOld::slerp2(const Eigen::Quaterniond& a, const Eigen::Quaterniond& b, const Eigen::Quaterniond& c, const double &t) {
    return slerp(slerp(a,b, t), slerp(b,c, t), t);
  }

  inline Eigen::Quaterniond
  NovaPathPlannerOld::slerp3(const Eigen::Quaterniond &a, const Eigen::Quaterniond &b, const Eigen::Quaterniond &c, const Eigen::Quaterniond &d,
                          const double &t) {
    return slerp(slerp2(a,b,c, t), slerp2(b,c,d, t), t);
  }

  inline Eigen::Isometry3d
  NovaPathPlannerOld::lerp3(Eigen::Isometry3d a, Eigen::Isometry3d b, Eigen::Isometry3d c, Eigen::Isometry3d d, double t) {
  ...
  inline std::vector<double> NovaPathPlannerOld::lerp(const std::vector<double>& a, const std::vector<double>& b, const double& t) {
...
CLASS_LOADER_REGISTER_CLASS(
    nova_path_planner_old::NovaPathPlannerOld, controller_interface::ControllerInterface)
