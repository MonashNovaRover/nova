...
> **Status (2026-04-09):** All ten steps complete. Steps 1–6 and 8–10 were
> executed in order; step 7 was initially skipped and then completed after
> step 10 as a follow-up. Standalone `test_manual` runner: 94/94 passing.
> Colcon build + URDF tests in `test/common/test_robot_model_urdf.cpp` and
> `test/joint_map/test_transmission_analysis_urdf_import.cpp` are deferred
> to a real ROS environment run.
...
4. ✅ **Build out the build-time analysis types.** Three pure-function pieces: (a) `TransmissionReachability` (in `transmission_reachability.hpp/.cpp`) — runs the eager forward fixed point given an analysis + inputs, exposes `producer_of`, `ambiguities`, `derivable_interfaces`, `redundant_equivalent_inputs`. (b) `diagnose_missing_outputs` (in `missing_input_resolution.hpp/.cpp`) — takes a reachability + needed outputs, returns a `MissingOutputDiagnosis`. (c) `JointMapBlueprint` + `plan_joint_map` (in `joint_map_blueprint.hpp/.cpp`) — takes a reachability + ordered outputs, materializes an immutable segment list (input/affine batches consolidated, transmission stages topologically ordered) ready for direct joint map construction. Tests live in `test/joint_map/test_transmission_reachability.cpp` and `test/joint_map/test_joint_map_blueprint.cpp` against synthetic `TransmissionAnalysis` instances — no FK dependency.
5. ✅ **Reconnect runtime joint maps.** Give `TransmissionJointMap`, `AffineJointMap`, and `CompositeJointMap` constructors / factories that consume `JointMapBlueprintSegment` data directly — one constructor call per segment kind. The blueprint already did the affine batching consolidation, so a `TransmissionJointMap` segment with N rows becomes one `AffineJointMap` with N rows, not N separate maps. Hard constraint: do not reintroduce the legacy `TransmissionPlan` / `compile_*_plan_expected` scaffolding that step 2 deleted.
6. ✅ **Rewire `DefaultJointMapBuilder`.** Construct a `TransmissionReachability`, fail loudly on ambiguity (`reach.is_ambiguous()`), then call `diagnose_missing_outputs(reach, outputs)` to fail loudly on missing/ambiguous needed outputs (with `compute_missing_input_resolutions` for hints). Otherwise call `plan_joint_map(reach, outputs)` and walk the resulting segments to emit a runtime `JointMap`. Production `joint_map` code compiles from this step; the FK plugin and tests are still broken until later steps.
7. ✅ **Restructure ros2_control wrappers.** *(Originally skipped; completed as a follow-up after step 10.)* Refactored `src/joint_map/transmission_analysis_import.cpp` around a shared `Ros2ControlPluginCore` that loads the plugin once at import and probes `{position, velocity, acceleration} × {forward, inverse}` combos via fresh dummy try-configures + per-direction propagation calls. A small `Ros2ControlPluginTransmissionInstanceModel` is registered per supported combo (e.g. `"main_transmission/position[fwd]"`), each sharing the core via `shared_ptr`. `Ros2ControlPluginTransmissionCompute` now holds a preconfigured `ConfiguredTransmission` (heap-pinned because the plugin handles store raw pointers into its value vectors); `clone()` mints a new one via `core_->make_configured(...)` instead of reloading the plugin. The default position/velocity/acceleration projection rules are already populated by `TransmissionAnalysis`'s constructor — the importer does not register them. (Effort is opt-in and not probed.) Mimic import already uses joint-level `add_affine_transmission` (was done in step 1) and is unchanged.
...
9. ✅ **Test split + updates.** Create `test/joint_map/` directory. Split `test_eigen_fk_mapper.cpp` into focused files. Update all call sites to the new API. Add `test_missing_input_resolution.cpp` (`test_transmission_reachability.cpp` and `test_joint_map_blueprint.cpp` already exist from step 4). Update CMakeLists.txt.
10. ✅ **Build, run all tests.** *(Standalone `test_manual` runner only — 94/94 passing. Colcon-only URDF tests deferred to the user's next ROS env run.)*
