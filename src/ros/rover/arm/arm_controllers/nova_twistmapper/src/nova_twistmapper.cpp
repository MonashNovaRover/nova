...
controller_interface::CallbackReturn NovaTwistmapper::on_init()
{
  try
  {
    ...
    plugin_loader_ = arm_kinematics::PluginLoader(*get_node(), get_robot_description());
  }
...
controller_interface::CallbackReturn NovaTwistmapper::on_configure(const rclcpp_lifecycle::State&)
{
  ...
  twist_stamped_sub_ = get_node()->create_subscription<geometry_msgs::msg::TwistStamped>(
    ...
    [this, logger](const std::shared_ptr<geometry_msgs::msg::TwistStamped> msg) -> void
    {
      ...
      const auto new_frame_id = msg->header.frame_id.empty() ? params_.fallback_frame_id : msg->header.frame_id;
      ...
      received_twist_stamped_ptr_.set(msg);
    });
  subscriber_is_active_ = true;

  RCLCPP_INFO(logger, "Created twist stamped subscription");

  fk_ = plugin_loader_->make_fk();
  if (!fk_)
    return CallbackReturn::FAILURE;

  auto [tree, _] = fk_->make_tree(
    params_.joint_names,
    params_.kinematics_base_frame,
    arm_kinematics::FrameDefinitions{params_.kinematics_endeffector_frame});
  tree_ = std::move(tree);

  if (!tree_)
    return CallbackReturn::FAILURE;

  // TODO: Make it so we can check for failures in creating the collision manager
  collision_ = arm_kinematics::CollisionManager(*plugin_loader_, fk_, params_.joint_names);

  ik_ = plugin_loader_->make_ik();
  if (!ik_)
    return CallbackReturn::FAILURE;
...
Eigen::Isometry3f NovaTwistmapper::integrate_twist(const std::vector<double> &seed_state,
  ...
  Eigen::Isometry3f twist_frame;

  if (params_.publish_debug_frames) {
    publish_to_tf2(get_node()->get_clock()->now(), twist_frame.cast<double>(), std::string(get_node()->get_name()) + "_twist_frame");
  }

  // Extract data from the twist message into a usable format for math
  Eigen::Matrix<double, 6, 1> twist;
  Eigen::fromMsg(twist_msg, twist);
  Eigen::Vector3f twist_linear = twist.block<3, 1>(0, 0).cast<float>();
  Eigen::Vector3f twist_angular = twist.block<3, 1>(3, 0).cast<float>();

  // Construct a 4x4 matrix from the above linear and angular values, but as a displacement rather than velocity.
  Eigen::Isometry3f new_pose = current_target_pose;
  ...
  if (twist_angular_norm > EPSILON) {
    ...
    Eigen::AngleAxisf angular_diff(twist_angular_norm * period.seconds(), twist_frame.linear() * twist_angular / twist_angular_norm);
...
controller_interface::return_type NovaTwistmapper::update(const rclcpp::Time &time, const rclcpp::Duration &period)
{
  ...
  // TODO: What the fuck man -- what the hell does real time mean?
