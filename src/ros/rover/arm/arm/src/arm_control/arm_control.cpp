...
ArmControl::ArmControl() : Node("arm_control")
{
    ...
    resolver_sub = this->create_subscription<sensor_msgs::msg::JointState>(
        "/arm/joint_states", 10, std::bind(&ArmControl::resolver_callback, this, _1)
    );
...
void ArmControl::control_scheme_callback(const arm_interfaces::msg::ArmControlScheme::SharedPtr msg)
{
    control_scheme = *msg;
...
void ArmControl::zero_resolver_callback(rclcpp::Client<arm_interfaces::srv::StringTrigger>::SharedFuture future){
    ...
    if (response->success){
        ...
        RCLCPP_ERROR(this->get_logger(), "Failed to zero resolver %s", response->message.c_str());
    }
}
...
void ArmControl::arm_config_info_callback(
    __attribute__((unused)) const arm_interfaces::srv::ArmConfigInfo::Request::SharedPtr request,
    arm_interfaces::srv::ArmConfigInfo::Response::SharedPtr response
)
{
