...
class TestableNovaDiffDriveController : public nova_diff_drive_controller::NovaDiffDriveController
{
public:
  using NovaDiffDriveController::NovaDiffDriveController;
  std::shared_ptr<geometry_msgs::msg::TwistStamped> getLastReceivedTwist()
  {
    std::shared_ptr<geometry_msgs::msg::TwistStamped> ret;
    received_twist_msg_ptr_.get(ret);
    return ret;
  }
    ...
    const std::chrono::milliseconds & timeout = std::chrono::milliseconds(500))
  {
    ...
    wait_set.add_subscription(twist_subscriber_);
...
class TestNovaDiffDriveController : public ::testing::Test
{
protected:
  ...
  void SetUp() override
  {
    controller_ = std::make_unique<TestableNovaDiffDriveController>();
  ...
  const std::string controller_name = "test_nova_diff_drive_controller";
  std::unique_ptr<TestableNovaDiffDriveController> controller_;

  const std::vector<std::string> left_drive_names = {"left_wheel_joint"};
  const std::vector<std::string> right_drive_names = {"right_wheel_joint"};
  ...
  hardware_interface::StateInterface left_wheel_pos_state_{
    left_drive_names[0], HW_IF_POSITION, &position_values_[0]};
  hardware_interface::StateInterface right_wheel_pos_state_{
    right_drive_names[0], HW_IF_POSITION, &position_values_[1]};
...
TEST_F(TestNovaDiffDriveController, configure_fails_without_parameters)
{
...
TEST_F(TestNovaDiffDriveController, configure_fails_with_only_left_or_only_right_side_defined)
{
  const auto ret = controller_->init(controller_name);
  ASSERT_EQ(ret, controller_interface::return_type::OK);
...
TEST_F(TestNovaDiffDriveController, configure_fails_with_mismatching_wheel_side_size)
{
  const auto ret = controller_->init(controller_name);
  ASSERT_EQ(ret, controller_interface::return_type::OK);
...
TEST_F(TestNovaDiffDriveController, configure_succeeds_when_wheels_are_specified)
{
  const auto ret = controller_->init(controller_name);
  ASSERT_EQ(ret, controller_interface::return_type::OK);
...
TEST_F(TestNovaDiffDriveController, configure_succeeds_tf_test_prefix_false_no_namespace)
{
  const auto ret = controller_->init(controller_name);
  ASSERT_EQ(ret, controller_interface::return_type::OK);
...
TEST_F(TestNovaDiffDriveController, configure_succeeds_tf_test_prefix_true_no_namespace)
{
  const auto ret = controller_->init(controller_name);
  ASSERT_EQ(ret, controller_interface::return_type::OK);
...
TEST_F(TestNovaDiffDriveController, configure_succeeds_tf_blank_prefix_true_no_namespace)
{
  const auto ret = controller_->init(controller_name);
  ASSERT_EQ(ret, controller_interface::return_type::OK);
...
TEST_F(TestNovaDiffDriveController, configure_succeeds_tf_test_prefix_false_set_namespace)
{
...
TEST_F(TestNovaDiffDriveController, configure_succeeds_tf_test_prefix_true_set_namespace)
{
  std::string test_namespace = "/test_namespace";
...
TEST_F(TestNovaDiffDriveController, configure_succeeds_tf_blank_prefix_true_set_namespace)
{
  std::string test_namespace = "/test_namespace";
...
TEST_F(TestNovaDiffDriveController, activate_fails_without_resources_assigned)
{
  const auto ret = controller_->init(controller_name);
  ASSERT_EQ(ret, controller_interface::return_type::OK);
...
TEST_F(TestNovaDiffDriveController, activate_succeeds_with_pos_resources_assigned)
{
  const auto ret = controller_->init(controller_name);
  ASSERT_EQ(ret, controller_interface::return_type::OK);
...
TEST_F(TestNovaDiffDriveController, activate_succeeds_with_vel_resources_assigned)
{
  const auto ret = controller_->init(controller_name);
  ASSERT_EQ(ret, controller_interface::return_type::OK);

  controller_->get_node()->set_parameter(
    rclcpp::Parameter("drive_position_feedback", rclcpp::ParameterValue(false)));
  controller_->get_node()->set_parameter(
    rclcpp::Parameter("left_drive_names", rclcpp::ParameterValue(left_drive_names)));
  controller_->get_node()->set_parameter(
    rclcpp::Parameter("right_drive_names", rclcpp::ParameterValue(right_drive_names)));
...
TEST_F(TestNovaDiffDriveController, activate_fails_with_wrong_resources_assigned_1)
{
  const auto ret = controller_->init(controller_name);
  ASSERT_EQ(ret, controller_interface::return_type::OK);
...
TEST_F(TestNovaDiffDriveController, activate_fails_with_wrong_resources_assigned_2)
{
  const auto ret = controller_->init(controller_name);
  ASSERT_EQ(ret, controller_interface::return_type::OK);

  controller_->get_node()->set_parameter(
    rclcpp::Parameter("drive_position_feedback", rclcpp::ParameterValue(true)));
  controller_->get_node()->set_parameter(
    rclcpp::Parameter("left_drive_names", rclcpp::ParameterValue(left_drive_names)));
  controller_->get_node()->set_parameter(
    rclcpp::Parameter("right_drive_names", rclcpp::ParameterValue(right_drive_names)));
...
TEST_F(TestNovaDiffDriveController, cleanup)
{
  const auto ret = controller_->init(controller_name);
  ASSERT_EQ(ret, controller_interface::return_type::OK);
  ...
  controller_->get_node()->set_parameter(
    rclcpp::Parameter("right_drive_names", rclcpp::ParameterValue(right_drive_names)));
  controller_->get_node()->set_parameter(rclcpp::Parameter("steering_track", 0.4));
...
TEST_F(TestNovaDiffDriveController, correct_initialization_using_parameters)
{
  const auto ret = controller_->init(controller_name);
  ASSERT_EQ(ret, controller_interface::return_type::OK);
