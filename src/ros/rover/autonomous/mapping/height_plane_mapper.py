...
class HeightMapper(Mapper):
    def __init__(self, length=20, width=20, height=5, resolution=0.1, detection_resolution=0.025, planner=None):
        ...
        self.height_mapper = HeightMapper(length, width, height, resolution, detection_resolution)
        self.plane_mapper = PlaneMapper(length, width, height, resolution, detection_resolution)
        self.initialise_map()
    ...
    def get_plane_obstacles(self, filtered_indices):
        return get_plane_obstacles(filtered_indices, self.detection_length, self.detection_width, self.resolution_raito)
    ...
    def publish(self, pts):
        """
        Publish the 2d maps over ros to be viewed in RVIZ
        """
