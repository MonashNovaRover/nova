...
class Controller(Node):
    ...
    def __init__(self):
        ...
        self.planners = {
            ...
            PlanningState.GATE_HONING: self.get_gate_goal,
            # cheeky lambda 
            PlanningState.SUCCESS: None,  # lambda: self.get_logger().fatal("Why is SUCCESS being called?")
            PlanningState.RETURN: self.get_return_goal
        }
    ...
    def planning_mode_state_transition(self):
        ...
        if self.planning_state.state == PlanningState.SUCCESS:
        ...
        elif self.planning_state.state == PlanningState.RETURN:
            ...
            elif self.ar_tag_manager.found_current_goals() and len(self.ar_tag_manager.ar_tag_goals) == 1:
                self.on_state_update(PlanningState.AR_HONING)

            elif self.ar_tag_manager.found_current_goals() and len(self.ar_tag_manager.ar_tag_goals) == 2:
                self.on_state_update(PlanningState.GATE_HONING)

        elif self.planning_state.state == PlanningState.SEARCH:

            if self.ar_tag_manager.found_current_goals() and len(self.ar_tag_manager.ar_tag_goals) == 1:
                self.on_state_update(PlanningState.AR_HONING)

            elif self.ar_tag_manager.found_current_goals() and len(self.ar_tag_manager.ar_tag_goals) == 2:
                self.on_state_update(PlanningState.GOAL_HONING)
        ...
        elif self.planning_state.state == PlanningState.GATE_HONING:

            if self.gate_array_index == len(self.gate_goals):
                self.on_state_update(PlanningState.SUCCESS)
    ...
    def get_current_goal_type(self):
        ...
        2 tags - rough location + two tags, we need to travel between the two tags as this is a GATE_HONING.
        """
    ...
    def callback_return(self, request, response):
    ...
    def callback_planner_path(self, msg):
        ...
        self.get_logger().debug(f"recceived path with {len(msg.waypoints)} waypoints")
    ...
    def get_current_goal(self) -> tuple:
        return self.waypoint_path[-1] if len(self.waypoint_path) > 0 else None
    ...
    def near_current_goal(self) -> bool:
        ...
        end_of_path = self.get_current_goal()
        if end_of_path is None:
            return True  # path is only empty if
        if self.planning_state.state != PlanningState.RETURN:
            return distance(
    ...
    def plan(self):
        ...
        self.get_logger().debug(
            f"Calling planner {self.planners[self.planning_state.state]} for state {self.planning_state.state}"
        )
