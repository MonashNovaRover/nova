...
void JoystickThrustmaster::check_valid_inputs() {
    ...
    if (valid_ax_stick_twist && valid_ax_thumb_y && valid_ax_slider) return;
    ...
    if (!valid_ax_thumb_y) {
        valid_ax_thumb_y = to_int(-convert_trg2ax(GamepadTriggerLength(controller, TRIGGER_RIGHT))) == 0.0;
    }
