...
class NIRProbePublisher(Node):
    ...
    def read_data_callback(self, frame: jcan.Frame):
        ...
        self.last_reading = int.from_bytes(frame.data, "big")
    ...
    def command_service_callback(self, request, response):
        ...
        message = None
        if request.led == self.PHOTODIODE1_ON:
            message = self.TURN_LED1_ON
        elif request.led == self.PHOTODIODE2_ON:
            message = self.TURN_LED2_ON

        if message is None:
            self.get_logger().error(f"Invalid NIR probe request made: {request.led}")
            response.success = False
            return response

        # Turn LED and photodiode on
        self.get_logger().info(f"Turning on NIR probe LED and photodiode {request.led}")

        frame = jcan.Frame(self.NIR_PROBE_ID, [message])
        self.active_photodiode = request.led
