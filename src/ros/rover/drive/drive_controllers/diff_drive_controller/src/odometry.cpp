...
bool Odometry::update(
  double left_drive_feedback, double right_drive_feedback, const rclcpp::Time& time)
{
  ...
  if (params_->drive_position_feedback)
  {
    ...
    double left_drive_pos = left_drive_feedback * left_wheel_radius_;
    double right_drive_pos = right_drive_feedback * right_wheel_radius_;
  ...
  else
  {
    ...
    left_speed = left_drive_feedback * left_wheel_radius_;
    right_speed = right_drive_feedback * right_wheel_radius_;
  }

  linear_velocity = (left_speed + right_speed) * 0.5;
  angular_velocity = (right_speed - left_speed) / wheel_separation_;
...
void Odometry::integrate_runge_kutta_2(double delta_linear, double delta_angular)
{
  ...
  x_ += delta_linear * cos(direction);
  y_ += delta_linear * sin(direction);
