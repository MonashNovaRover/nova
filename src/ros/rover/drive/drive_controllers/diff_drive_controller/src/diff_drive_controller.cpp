  ...
  const rclcpp::Time& time, const rclcpp::Duration& period)
{
  ...
  else
  {
    ...
    for (size_t index = 0; index < static_cast<size_t>(params_.wheels_per_side); ++index)
    {
      ...
      const double right_feedback =
        registered_right_drive_handles_.at(index).state.get().get_value();

      if (std::isnan(left_feedback) && std::isnan(right_feedback))
      {
        RCLCPP_ERROR(
          logger, "Either the left or right wheel %s is invalid for index [%zu]",
          drive_feedback_type(), index);
        return controller_interface::return_type::ERROR;
      }
  ...
  else
  {
    ...
    for (size_t index = 0; index < static_cast<size_t>(params_.wheels_per_side); ++index)
    {
      ...
      RCLCPP_INFO(
        logger, "Left wheel speed: %f, Right wheel speed: %f", left_speed / params_.wheel_radius,
        right_speed / params_.wheel_radius);
    }
  }
  ...
  const rclcpp_lifecycle::State&)
{
  ...
  {
    limited_twist_publisher_ = get_node()->create_publisher<geometry_msgs::msg::TwistStamped>(
