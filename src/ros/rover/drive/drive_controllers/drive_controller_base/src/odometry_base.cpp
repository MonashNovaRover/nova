...
#include "drive_controller_base/odometry_base.hpp"
...
OdometryBase::OdometryBase(
  rclcpp_lifecycle::LifecycleNode::SharedPtr node, std::shared_ptr<Params> params)
  : node_(std::move(node))
...
void OdometryBase::update_open_loop(double linear, double angular, const rclcpp::Time& time)
{
...
void OdometryBase::publish(const rclcpp::Time& time)
{
...
void OdometryBase::integrate_runge_kutta_2(double delta_linear, double delta_angular)
{
...
void OdometryBase::integrate_exact(double delta_linear, double delta_angular)
{
