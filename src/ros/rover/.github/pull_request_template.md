## PR Rules
1. Test the changes on the rover (if applicable) 
2. Merge (or rebase) the 'testing' branch into this branch before PR merge
3. Add [WIP] at the start of the PR title if you're still working on the change
4. Delete this branch after PR merge

## PR Checklist
- [ ] Tested on the rover (if applicable)
- [ ] Assigned reviewer
- [ ] Included unit tests (if necessary)
- [ ] Merged/rebased testing into this branch
- [ ] Moved task on trello to "code reveiw"

Remove an option if irrelevant to your changes

## PR Details

### Summary of the PR

<!-- Summary of changes. Good rule of thumb: If I merge this PR it will ....  -->

<hr/>

### Reason for change

<hr/>

### Link any branch/PR/issue this change depends on

<hr/>

### Screenshots (If applicable)

<hr/>

### Please explain anything that can be helpful to the reviewers since they didn't write the code themselves.

<hr/>

### Testing details (if applicable)
<!-- What needs to be tested?
How to test the changes?
Are there existing/new unit tests that test the changes/parts of it?
-->

<hr/>

Remove a prompt if irrelevant to your changes
