export default class EffectQueue<T extends unknown[]> {
  ...
  /**
   * Pushes an effect to the callback queue, and overwrites any existing request with the given id
   * @param frameID The result of this function, or this.push
   * @param effect The effect to run when the queue is cleared
   */
  public update(frameID: number, effect?: (...args: T) => void) {
    ...
    if (index > this.queue.length) {
      console.error("EffectQueue.update was given an index that wasn't generated by EffectQueue.push!");
      return -1;
    }

    this.queue[index] = effect;
    return index;
  }
