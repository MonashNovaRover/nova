...
import { Button, Card, CardHeader, Input, Modal, ModalBody, ModalDialog, ModalHeader, useOverlayState } from "@heroui/react";
...
const RamanCCDInputs: React.FC = () => {
    ...
    return (
        <Card className="h-40 m-1 p-5 flex flex-row flex-wrap flex-1 space-x-10">
            ...
            <Button isIconOnly className="w-8 h-8 m-3" onPress={overlayState.open}>
            ...
            <Input aria-label="Port" onChange={(event) => setPort(event.target.value)} className="shrink-0 w-44 grow" type="text" placeholder="Example: /dev/ttyACM0" defaultValue={port} />
            <Input aria-label="SH Period" onChange={(event) => setSHPeriod(+event.target.value)} className="shrink-0 w-36 grow" type="number" placeholder="[20, 4294967295]" defaultValue={shPeriod.toString()} />
            <Input aria-label="ICG Period" onChange={(event) => setICGPeriod(+event.target.value)} className="shrink-0 w-40 grow" type="number" placeholder="[14776, 4294967295]" defaultValue={icgPeriod.toString()} />
            <Input aria-label="Average" onChange={(event) => setAverage(+event.target.value)} className="shrink-0 w-16 grow" type="number" placeholder="[1, 15]" defaultValue={average.toString()} />
            <Button 
            onPress={() => {setSingleCollectionMode(!singleCollectionMode)}}
            variant={singleCollectionMode ? "primary" : "secondary"} className="h-14 shrink-0 w-52">
            ...
            }} variant={currentlyInContinuous ? "danger" : "primary"} className="h-14 flex flex-row shrink-0 grow w-32">{currentlyInContinuous ? "Stop" : "Collect"}</Button>
        </Card>
    )
}
