...
class GPSBase(Node):
    def __init__ (self):
        ...
        self.reader_rate = self.declare_parameter(
            name='reader_rate',
            value=100,
        ).value
        self.fix_type = None
        ...
        ### ROS2 ###
        self.publisher_cb_group = MutuallyExclusiveCallbackGroup()
        self.reader_cb_group = MutuallyExclusiveCallbackGroup()
        ...
        self.publish_timer = self.create_timer(1/self.publisher_rate, self.publish_msgs, callback_group=self.publisher_cb_group)
        self.reader_timer = self.create_timer(1/self.reader_rate, self.parse_message, callback_group=self.reader_cb_group)

        self.rtcm_msg_raw = None
        self.lock = Lock()
    ...
    def publish_msgs(self) -> None:
        with self.lock:
            ...
            if self.rtcm_msg_raw is not None:
                msg_binary = UInt8MultiArray()
                msg_binary.data = list(self.rtcm_msg_raw)
                self.pub_rtcm.publish(msg_binary)
            else:
                self.get_logger().warn(f'Cannot publish RTCM message as connected GPS has never provided an RTCM message', throttle_duration_sec=2)
