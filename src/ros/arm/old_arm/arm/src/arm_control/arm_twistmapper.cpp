...
#include "arm_control/arm_twistmapper.h"

#include "arm_core/arm_messages.h"
#include "arm_core/arm_type_translation.h"
...
{    
    ...
    resolver_sub = this->create_subscription<sensor_msgs::msg::JointState>(
        "/arm/joint_states", 10, std::bind(&ArmTwistMapper::resolver_callback, this, _1)
    );
    ...
    std::cout << C_TITLE << "ARM CONFIGURATION" << C_END "\n";
    std::cout << "Wrist:\n";
    std::cout << module_names_upper[1].c_str();
...
void ArmTwistMapper::control_scheme_callback(const arm_interfaces::msg::ArmControlScheme::SharedPtr msg)
{
...
inline KDL::Twist ArmTwistMapper::get_control_twist(const KDL::Twist& joystick_twist)
{
    ...
    // Apply input transforms to convert from joystick directions to end-effector frame coordinates
    if (control_scheme.endpoint_frame_linear) {
        twist_linear = ENDPOINT_INPUT_TRANSFORM_LINEAR * twist_linear;
    }
    if (control_scheme.endpoint_frame_angular) {
        twist_angular = ENDPOINT_INPUT_TRANSFORM_ANGULAR * twist_angular;
    }
    ...
    if (control_scheme.endpoint_frame_linear || control_scheme.endpoint_frame_angular){
        ...
        if (control_scheme.endpoint_frame_linear) {
            twist_linear = endpoint_coord_transform_linear * twist_linear;
        }
        if (control_scheme.endpoint_frame_angular) {
            twist_angular = endpoint_coord_transform_angular * twist_angular;
        }
    }
