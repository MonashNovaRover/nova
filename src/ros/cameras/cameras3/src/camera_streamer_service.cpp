...
class CameraStreamer : public rclcpp::Node
{
  ...
  private: void start_pipeline(Pipeline* pipeline)
  {
    ...
    } else if (pipeline->pipeline_type == "v4lrostopic") {
      v4lrostopicPipelineProperties* props = get_v4lrostopic_pipeline_properties(this, pipeline->camera, pipeline->pipeline_type, pipeline->profile);
      pipeline->props = props;
