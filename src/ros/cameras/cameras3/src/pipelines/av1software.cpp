...
av1softwarePipelineProperties* get_av1software_pipeline_properties(rclcpp::Node* streamer_node, camera_msgs::msg::Camera* camera)
{
  ...
  // source
  props->strict_devname = set_property(streamer_node, camera->serial, camera->profile, camera->original_serial, "strict_devname", true);
  props->device =  (props->strict_devname) ? set_property(streamer_node, camera->serial, camera->profile, camera->original_serial, "device", camera->node) : camera->node;
