...
GstElement* vp9software_pipeline(rclcpp::Node* streamer_node, vp9softwarePipelineProperties* props)
{
  ...
  if (!gst_pipeline || !source || (props->downrate > 1 && !rate) || !srcfilter || (props->mime == "image/jpeg" && !decode) || (props->greyscale && !greyconvert && !greyfilter) || !convert || !scalefilter  || (props->show_clock && !clock) || (props->crop43 && !cropper) || !encode || !webrtc) {
  ...
  set_vp9enc(encode, props->cpu_used, props->noise, props->threads, props->bitrate, props->gop, props->framerate, props->framerate_denominator, props->downrate);
...
{
  // 0. Initialize constants
  vp9softwarePipelineProperties* props = new vp9softwarePipelineProperties;
  ...
  default_string = "mmap";
  props->io_mode = set_property(streamer_node, camera->serial, camera->profile, camera->original_serial, "io_mode", "mmap");
  ...
  props->decoder = set_property(streamer_node, camera->serial, camera->profile, camera->original_serial, "decoder", default_string);
  default_string = "ifast";
  props->jpegdec_method = set_property(streamer_node, camera->serial, camera->profile, camera->original_serial, "jpegdec_method", default_string);

  // greyscale
  props->greyscale = set_property(streamer_node, camera->serial, camera->profile, camera->original_serial, "greyscale", false);
  ...
  // encode
  props->cpu_used = set_property(streamer_node, camera->serial, camera->profile, camera->original_serial, "cpu_used", 1);
  props->gop = set_property(streamer_node, camera->serial, camera->profile, camera->original_serial, "gop", 1);
  props->noise = set_property(streamer_node, camera->serial, camera->profile, camera->original_serial, "noise", 0);
  props->threads = set_property(streamer_node, camera->serial, camera->profile, camera->original_serial, "threads", 1);
