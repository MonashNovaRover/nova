...
Map2D get_2d_map(Map3D& map3D){
    ...
    for (int plane_i = 0; plane_i < num_planes_x; plane_i++) {
        for (int plane_j = 0; plane_j < num_planes_y; plane_j++) {
            Vec3 point_sum;
            std::vector<Vec3> these_pts(PIXELS_PER_PLANE2);
            for (int i = 0; i < PIXELS_PER_PLANE; i++) {
                for (int j = 0; j < PIXELS_PER_PLANE; j++) {
                    for (int k = 0; k < (int) ZS; k++){
                        // Careful - this assumes x, y and z resolutions are all the same.
                        // Need to do account for pixel dimensions if we re-scale any of these
                        Vec3 p(i, j, k);
